...
import numpy as np
import Utils_losses
import tensorflow_probability as tfp   #--> conda install -c hcc tensorflow-probability

EPSILON = 1e-10  #--> a very small number

class ResNet_Siamese(object):

    def __init__(self, loss_type, feature_space_dimension, n_triplets_per_batch, n_classes, n_samples_per_class_in_batch, n_res_blocks=18, margin_in_loss=0.25, batch_size=32):
        ...
        elif self.loss_type == "Distribution_updating_triplet_loss":
            self.loss = self.Distribution_updating_triplet_loss(labels=self.labels1, embeddings=self.o1, margin=self.margin_in_loss, NCA_mode=False, squared=True)
        elif self.loss_type == "Distribution_updating_NCA_loss":
            self.loss = self.Distribution_updating_triplet_loss(labels=self.labels1, embeddings=self.o1, margin=self.margin_in_loss, NCA_mode=True, squared=True)
    ...
    # def Distribution_updating_triplet_loss_HAS_ERROR(self, labels, embeddings, margin, NCA_mode, squared=False):
    #
    #     # update the sample size so far:
    #     if not self.is_first_batch:
    #         self.n_samples_per_class_so_far += (self.batch_size // self.n_classes)
    #     n_samples_new_per_class = (self.batch_size // self.n_classes)
    #
    #     covariance_of_classes = tf.Variable(tf.zeros(shape=(self.feature_space_dimension, self.feature_space_dimension, self.n_classes), dtype=tf.float32))  #--> https://stackoverflow.com/questions/47775067/how-to-assign-the-element-of-tensor-from-other-tensor-in-tensorflow
    #     mean_of_classes = tf.Variable(tf.zeros(shape=(1, self.feature_space_dimension, self.n_classes), dtype=tf.float32))
    #     for class_index in range(self.n_classes):
    ...
    #             covariance_, mean_ = Utils_losses.tf_covariance_matrix(x=samples_of_this_class)
    #             # update mean of class:
    #             tf.assign(mean_of_classes[:, :, class_index], mean_)
    #             # update covariance of class:
    #             tf.assign(covariance_of_classes[:, :, class_index], covariance_)
    #         else:
    #             covariance_new, mean_new = Utils_losses.tf_covariance_matrix(x=samples_of_this_class, scaled_covariance=False)
    #             # update mean of class:
    #             expectation_of_normal_dist = ((self.n_samples_per_class_so_far * self.mean_prior[:, :, class_index]) +
    #                                          (n_samples_new_per_class * mean_new)) / (self.n_samples_per_class_so_far + n_samples_new_per_class)
    #             tf.assign(mean_of_classes[:, :, class_index], expectation_of_normal_dist)
    #             # update covariance of class:
    #             df_inverseWishart = self.n_samples_per_class_so_far + n_samples_new_per_class
    #             Psi_ = self.n_samples_per_class_so_far * self.covariance_prior[:, :, class_index]
    #             scale_matrix_inverseWishart = Psi_ + covariance_new + \
    #                                           (((self.n_samples_per_class_so_far * n_samples_new_per_class) / (self.n_samples_per_class_so_far + n_samples_new_per_class)) *
    #                                            tf.matmul(tf.transpose(mean_new - self.mean_prior[:, :, class_index]), mean_new - self.mean_prior[:, :, class_index]))
    #             if df_inverseWishart > (self.feature_space_dimension + 1):
    #                 expectation_of_inverseWishart_dist = scale_matrix_inverseWishart / (df_inverseWishart - self.feature_space_dimension - 1)
    #                 tf.assign(covariance_of_classes[:, :, class_index], expectation_of_inverseWishart_dist)
    #             else:
    #                 covariance_new, _ = Utils_losses.tf_covariance_matrix(x=samples_of_this_class, scaled_covariance=True)
    #                 tf.assign(covariance_of_classes[:, :, class_index], covariance_new)
    #
    #     # update the priors:
    #     self.covariance_prior = covariance_of_classes
    #     self.mean_prior = mean_of_classes
    #
    #     # calculate anchor_positive_dist and anchor_negative_dist:
    #     anchor_positive_dist = tf.Variable(tf.zeros(shape=(self.batch_size, self.n_classes-1), dtype=tf.float32))
    #     anchor_negative_dist = tf.Variable(tf.zeros(shape=(self.batch_size, self.n_classes-1), dtype=tf.float32))
    #     for sample_index in range(self.batch_size):
    #         label_of_sample = labels[sample_index]
    #         anchor_ = tf.reshape(embeddings[sample_index, :], shape=[1, self.feature_space_dimension])
    #         # anchor_positive_dist:
    #         class_distribution = tfp.distributions.MultivariateNormalFullCovariance(loc=mean_of_classes[:, :, label_of_sample], covariance_matrix=covariance_of_classes[:, :, label_of_sample])
    #         n_positive_samples = self.n_classes-1
    #         positive_sampling = tfp.distributions.Sample(distribution=class_distribution, sample_shape=n_positive_samples)  #--> shape: (sample_shape, self.feature_space_dimension)
    #         positive_samples = positive_sampling.sample()
    #         positive_samples = tf.reshape(positive_samples, shape=[n_positive_samples, self.feature_space_dimension])  #--> remove the dummy dimension(s)
    #         the_anchor_positive_dist_ = Utils_losses.distance_of_embeddings(embeddings1=anchor_, embeddings2=positive_samples, squared=squared, l2_normalization=False)
    #         tf.assign(anchor_positive_dist[sample_index, :], the_anchor_positive_dist_)
    #         del class_distribution
    #         del positive_sampling
    #         # anchor_negative_dist:
    #         negative_samples = tf.Variable(tf.zeros(shape=(self.n_classes, self.feature_space_dimension), dtype=tf.float32))
    #         for class_index in range(self.n_classes):
    #             class_distribution = tfp.distributions.MultivariateNormalFullCovariance(loc=mean_of_classes[:, :, class_index], covariance_matrix=covariance_of_classes[:, :, class_index])
    #             negative_sampling = tfp.distributions.Sample(distribution=class_distribution, sample_shape=1)  #--> shape: (sample_shape, self.feature_space_dimension)
    #             negative_sample_ = negative_sampling.sample()
    #             negative_sample_ = tf.reshape(negative_sample_, shape=[1, self.feature_space_dimension])  #--> remove the dummy dimension(s)
    #             tf.assign(negative_samples[class_index, :], negative_sample_)
    #             del class_distribution
    #             del negative_sampling
    #         the_anchor_negative_dist_ = Utils_losses.distance_of_embeddings(embeddings1=anchor_, embeddings2=negative_samples, squared=squared, l2_normalization=False)
    #         # remove the invalid negative sample (which belongs to the same class as the anchor):
    #         label_of_sample_repeated = tf.ones(shape=[self.n_classes], dtype=tf.int32) * label_of_sample
    #         mask_for_class_of_this_sample = tf.math.equal(tf.range(start=0, limit=self.n_classes, delta=1), label_of_sample_repeated)
    #         the_anchor_negative_dist_masked = tf.boolean_mask(tensor=the_anchor_negative_dist_, mask=tf.math.logical_not(mask_for_class_of_this_sample), axis=1)
    #         tf.assign(anchor_negative_dist[sample_index, :], the_anchor_negative_dist_masked)
    #
    #     # loss function:
    #     if not NCA_mode:
    ...
    #         numerator_ = tf.math.exp(-1 * anchor_positive_dist)
    #         denominator_ = tf.reduce_sum(tf.math.exp(-1 * anchor_negative_dist), axis=1, keep_dims=True)
    #         denominator_ = tf.tile(denominator_, tf.constant([1, self.n_classes-1], tf.int32))
    #         denominator_ += EPSILON
    #         triplet_loss = -1 * tf.math.log(tf.math.divide(numerator_, denominator_) + EPSILON)
    #         triplet_loss = tf.reduce_mean(triplet_loss)
    #
    ...
    def Distribution_updating_triplet_loss(self, labels, embeddings, margin, NCA_mode, squared=False):
        ...
        for class_index in range(self.n_classes):
            ...
            # calculate priors:
            if self.is_first_batch:
                if class_index == (self.n_classes - 1):
                    self.is_first_batch = False
                covariance_, mean_ = Utils_losses.tf_covariance_matrix(x=samples_of_this_class)
                # update mean of class:
                tf.assign(mean_of_classes[:, :, class_index], mean_)
                ...
                scale_matrix_inverseWishart = Psi_ + covariance_new + \
                                              (((self.n_samples_per_class_so_far * n_samples_new_per_class) / (self.n_samples_per_class_so_far + n_samples_new_per_class)) *
                                               tf.matmul(tf.transpose(mean_new - self.mean_prior[:, :, class_index]), mean_new - self.mean_prior[:, :, class_index]))
                if df_inverseWishart > (self.feature_space_dimension + 1):
                    expectation_of_inverseWishart_dist = tf.linalg.inv(scale_matrix_inverseWishart) / (df_inverseWishart - self.feature_space_dimension - 1)
                    tf.assign(covariance_of_classes[:, :, class_index], expectation_of_inverseWishart_dist)
                else:
                    covariance_new, _ = Utils_losses.tf_covariance_matrix(x=samples_of_this_class, scaled_covariance=True)
                    tf.assign(covariance_of_classes[:, :, class_index], covariance_new)
